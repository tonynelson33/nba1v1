...
- `ShareImageButton` (rendered from `BracketApp`, label "Share Bracket") is absolutely positioned with **hardcoded pixel offsets** (`left-[826px] top-[158px]`) to sit centered above the Wings region banner, level with the Overall Champion box — a user request for exact placement, not arbitrary. This only stays correct because every bracket dimension (`Column`'s `w-40`, the semifinal/final column widths, all the gaps) is a fixed pixel value with no responsive variation — the bracket's total rendered size never changes. If you make any of those dimensions responsive or data-dependent, this button's position (and the `mb-12` gap above the amber box that makes room for it, in `Bracket.tsx`) will need to be re-measured, not just left alone.
